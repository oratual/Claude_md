...
func (t *TmuxSession) CapturePaneContent() (string, error) {
	// Check if we're in a non-tmux environment (like Windows Terminal via WSL)
	if os.Getenv("TMUX") == "" {
		// Check if tmux server is running
		checkCmd := exec.Command("tmux", "list-sessions")
		if err := checkCmd.Run(); err != nil {
			// No tmux server running - return empty content without error
			// This allows Claude Squad to work without pane capture
			return "", nil
		}
	}
	...
	if err != nil {
		// Check for common tmux errors and handle gracefully
		errStr := err.Error()
		if strings.Contains(errStr, "no server running") ||
		   ...
		   strings.Contains(errStr, "no pane") {
			// Expected errors when tmux isn't available - return empty
			log.InfoLog.Printf("tmux capture-pane: tmux not available (expected in Windows Terminal/WSL2)")
			return "", nil
		}
		
		// For other unexpected errors, log more details for debugging
		log.ErrorLog.Printf("tmux capture-pane unexpected error: %v", err)
		log.ErrorLog.Printf("tmux session name: %s", t.sanitizedName)
		log.ErrorLog.Printf("TMUX env var: %s", os.Getenv("TMUX"))
		log.ErrorLog.Printf("Terminal type: %s", os.Getenv("TERM"))
		
		// Try to get more info about tmux state
		if listCmd := exec.Command("tmux", "list-sessions"); listCmd != nil {
			if output, _ := listCmd.Output(); len(output) > 0 {
				log.InfoLog.Printf("Current tmux sessions: %s", string(output))
			}
		}
		
		// Still return empty to avoid breaking Claude Squad
		return "", nil
	}
...
func (t *TmuxSession) CapturePaneContentWithOptions(start, end string) (string, error) {
	// Check if we're in a non-tmux environment
	if os.Getenv("TMUX") == "" {
		checkCmd := exec.Command("tmux", "list-sessions")
		if err := checkCmd.Run(); err != nil {
			return "", nil
	...
	if err != nil {
		...
		log.InfoLog.Printf("tmux capture-pane warning: %v", err)
		return "", nil
	}
	return string(output), nil
}
