...
class BatmanIncorporated:
    ...
    def _analyze_and_plan(self, task_description: str) -> List[Task]:
        ...
        """
        # Cargar módulo apropiado según la tarea
        self.active_module = self.module_loader.get_module_for_task(task_description)
        if self.active_module:
            self.logger.log(f"📦 Módulo '{self.active_module.name}' cargado para esta tarea")
            # Mejorar agentes con capacidades del módulo
            self._enhance_agents_with_module()
    ...
    def _enhance_agents_with_module(self):
        """Mejora los agentes con herramientas del módulo activo."""
        if not self.active_module:
            return
            
        enhancements = self.active_module.get_agent_enhancements()
        
        for agent_name, agent in self.agents.items():
            if agent_name in enhancements:
                enhancement = enhancements[agent_name]
                # Agregar información del módulo al agente
                agent.module_tools = self.active_module
                agent.module_enhancement = enhancement
                self.logger.log(f"  • {agent_name} mejorado con capacidades de {self.active_module.name}")